terraform {
  backend "gcs" {
    bucket = "testies2-terraform"
    prefix = "terraform/testies2"
  }
}
provider "google" {
  project = var.project
...
module "testies2-service-account" {
...
resource "google_storage_bucket_iam_binding" "binding" {
  ...
  members = [
    "serviceAccount:${var.k8s_project}.svc.id.goog[${var.namespace}/${module.testies2-service-account.name}]"
  ]
}
